...
async function studentStats(userId) {
  ...
  return { assignmentsCompleted: completed, assignmentsSubmitted: projects.length, avgScore, recentProjects: recent };
}
...
async function teacherStats(teacherId) {
  ...
  const studentIds = await Project.distinct('studentId', { assignmentId: { $in: myAssignmentIds } });
  const totalStudents = studentIds.length;
  const avgAgg = await Project.aggregate([
    { $match: { assignmentId: { $in: myAssignmentIds } } },
    { $lookup: { from: 'ratings', localField: '_id', foreignField: 'projectId', as: 'r' } },
    { $unwind: '$r' },
  ...
  return { totalStudents, totalAssignments, avgClassPerformance };
}
...
async function studentActivity(userId) {
  ...
  for (const p of projects) {
    items.push({ id: `sub-${p._id}`, type: 'submission', title: p.title, subtitle: p.assignmentId?.title || '', at: p.createdAt, href: `/project/${p._id}` });
  }
  const comments = await Comment.find({ userId: uid }).sort({ createdAt: -1 }).limit(6).populate({ path: 'projectId', select: 'title' }).lean();
  for (const c of comments) {
    items.push({ id: `com-${c._id}`, type: 'comment', title: (c.message || '').slice(0, 80) + ((c.message || '').length > 80 ? '…' : ''), subtitle: c.projectId?.title || '', at: c.createdAt, href: c.projectId ? `/project/${c.projectId._id}` : undefined });
  }
  const pidsForGrades = await Project.find({ studentId: uid }).distinct('_id');
  if (pidsForGrades.length) {
    const ratings = await Rating.find({ projectId: { $in: pidsForGrades }, finalScore: { $ne: null } })
      .sort({ gradedAt: -1 })
      .limit(8)
      .populate({ path: 'projectId', select: 'title assignmentId studentId', populate: { path: 'assignmentId', select: 'title' } })
      .lean();
    for (const r of ratings) {
      if (!r.projectId) continue;
      items.push({ id: `grade-${r._id}`, type: 'grade', title: r.projectId.assignmentId?.title || r.projectId.title || 'Graded', subtitle: String(r.finalScore), at: r.gradedAt || r.updatedAt, href: `/project/${r.projectId._id}` });
    }
  }
...
async function teacherActivity(teacherId) {
  ...
  const subs = await Project.find({ assignmentId: { $in: myAssignmentIds } })
    ...
    .lean();
  for (const p of subs) {
    items.push({ id: `sub-${p._id}`, type: 'submission_received', title: p.title, subtitle: `${p.studentId?.name || 'Student'} · ${p.assignmentId?.title || ''}`, at: p.createdAt, href: `/project/${p._id}` });
  }
  const graded = await Rating.find({ finalScore: { $ne: null } })
    .sort({ gradedAt: -1 })
    .limit(15)
    .populate({ path: 'projectId', select: 'title assignmentId studentId', populate: [{ path: 'assignmentId', select: 'title createdBy' }, { path: 'studentId', select: 'name' }] })
    .lean();
  for (const r of graded) {
    const a = r.projectId?.assignmentId;
    if (!a || String(a.createdBy) !== String(tid)) continue;
    items.push({ id: `grade-${r._id}`, type: 'grade_given', title: r.projectId?.title || 'Project', subtitle: `${r.projectId?.studentId?.name || 'Student'} → ${r.finalScore}`, at: r.gradedAt || r.updatedAt, href: r.projectId ? `/project/${r.projectId._id}` : undefined });
  }
  items.sort((a, b) => new Date(b.at) - new Date(a.at));
  return items.slice(0, 12);
}
...
export const getProfile = asyncHandler(async (req, res) => {
  ...
  if (user.role === 'student') {
    ...
    return ok(res, { user: base, stats, activity, profileCompletion: completion });
  }
...
const updateSchema = z.object({
  ...
  notifications: z.object({ email: z.boolean().optional(), assignmentUpdates: z.boolean().optional(), comments: z.boolean().optional() }).optional(),
});
...
export const updateProfile = asyncHandler(async (req, res) => {
  ...
  if (body.email && body.email.toLowerCase() !== user.email) {
    const taken = await User.findOne({ email: body.email.toLowerCase(), _id: { $ne: user._id } }).lean();
  ...
  if (body.notifications) {
    ...
    user.notifications = {
      ...
      assignmentUpdates: n.assignmentUpdates !== undefined ? n.assignmentUpdates : cur.assignmentUpdates !== false,
      comments: n.comments !== undefined ? n.comments : cur.comments !== false,
    };
  }
  await user.save();
  return ok(res, { user: formatUser(user.toObject()) }, 'Profile updated');
});
...
export const updatePassword = asyncHandler(async (req, res) => {
  ...
  if (!isValid) return fail(res, 'Current password is incorrect', 400);
...
export const uploadAvatar = asyncHandler(async (req, res) => {
  ...
  return ok(res, { user: formatUser(user.toObject()) }, 'Avatar updated');
});
...
export const logoutAllDevices = asyncHandler(async (req, res) => {
  ...
  return ok(res, { token, user: formatUser(user) }, 'Other devices signed out. This session stays active.');
});
...
export const deleteAccount = asyncHandler(async (req, res) => {
  ...
  if (!isValid) return fail(res, 'Password is incorrect', 400);
  const uid = user._id;
  if (user.role === 'student') {
...
export const listStudents = asyncHandler(async (req, res) => {
  ...
  const myAssignmentIds = (await Assignment.find({ createdBy: teacherId }).select('_id').lean()).map((a) => a._id);
  const totalAssignments = myAssignmentIds.length;
  ...
  const data = await Promise.all(students.map(async (u) => {
    const completedIds = await Project.distinct('assignmentId', { studentId: u._id, assignmentId: { $in: myAssignmentIds } });
    const completed = completedIds.length;
    const pids = (await Project.find({ studentId: u._id, assignmentId: { $in: myAssignmentIds } }).select('_id').lean()).map((p) => p._id);
    let avgScore = null;
    if (pids.length) {
      const agg = await Rating.aggregate([{ $match: { projectId: { $in: pids }, finalScore: { $ne: null } } }, { $group: { _id: null, avg: { $avg: '$finalScore' } } }]);
      if (agg[0]?.avg != null) avgScore = Math.round(agg[0].avg);
    }
    return { user: formatUser(u), stats: { completedAssignments: Math.min(totalAssignments, completed), totalAssignments, avgScore } };
  }));
