...
export const getAnalyticsSummary = asyncHandler(async (req, res) => {
  ...
  const assignmentStatus = await Promise.all(
    byAssignment.map(async (a) => {
      ...
      return { name: a.title.length > 24 ? `${a.title.slice(0, 24)}…` : a.title, completed: submissions, pending };
    }),
  );

  const gradedRatings = await Rating.find({ finalScore: { $ne: null } })
    .populate({ path: 'projectId', select: 'assignmentId', populate: { path: 'assignmentId', select: 'createdBy' } })
    .lean();
  ...
  const teacherProjects = await Project.find({ assignmentId: { $in: myAssignmentIds } }).select('tools').lean();
  let ev3Only = 0; let tinkOnly = 0; let both = 0;
  for (const p of teacherProjects) {
    const t = p.tools || [];
    const hasE = t.includes('ev3'); const hasT = t.includes('tinkercad');
    if (hasE && hasT) both += 1; else if (hasE) ev3Only += 1; else if (hasT) tinkOnly += 1;
  }
  const toolUsage = [
  ...
  const topAgg = await Project.aggregate([
    ...
    { $unwind: '$r' },
    { $match: { 'r.finalScore': { $ne: null } } },
    { $group: { _id: '$studentId', avgScore: { $avg: '$r.finalScore' }, assignments: { $sum: 1 } } },
    { $sort: { avgScore: -1 } },
    { $limit: 10 },
    { $lookup: { from: 'users', localField: '_id', foreignField: '_id', as: 'u' } },
    { $unwind: '$u' },
  ]);
  const topStudents = topAgg.map((row, i) => ({ name: row.u.name, avgScore: row.avgScore != null ? Math.round(row.avgScore) : 0, assignments: row.assignments || 0, rank: i + 1 }));

  const eightWeeksAgo = new Date();
  eightWeeksAgo.setDate(eightWeeksAgo.getDate() - 56);
  const recentProjects = await Project.find({ assignmentId: { $in: myAssignmentIds }, createdAt: { $gte: eightWeeksAgo } })
    .populate({ path: 'assignmentId', select: 'createdBy', match: { createdBy: teacherId } })
    .lean();
  ...
  for (const p of recentProjects) {
    ...
    weekMap.get(wk).submissions += 1;
  }
  const ratingByProject = new Map((await Rating.find({}).lean()).map((r) => [String(r.projectId), r]));
  for (const p of recentProjects) {
    if (!p.assignmentId) continue;
  ...
  const performanceTrend = sortedWeeks.map((wk, idx) => {
    ...
    const avg = b.scores.length > 0 ? Math.round(b.scores.reduce((s, x) => s + x, 0) / b.scores.length) : 0;
    return { name: `W${idx + 1}`, avgScore: avg, submissions: b.submissions };
  });

  return ok(res, {
    stats: { totalStudents, myAssignments, pendingSubmissions, avgPerformance },
    assignmentStatus,
    gradeDistribution,
    toolUsage,
    topStudents,
